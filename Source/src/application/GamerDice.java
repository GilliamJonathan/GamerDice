...
public class GamerDice extends Application implements Serializable{
	...
	static String filePath = "C:/GamerDice/";
	static ArrayList<String> directorys;
	static ArrayList<Game> games;
	static String steamDirectory;
	public String version = "Alpha 1.01";
    ...
    public void start(Stage primaryStage){
        try {
            Pane page = (Pane) FXMLLoader.load(GamerDice.class.getResource("GamerDice.fxml"));
	...
	static Game roll(ArrayList<Game> g){
		if (g.size() == 1)
			return g.get(0);
		if (g.size() == 0)
			return null;
	...
	static void addDirectory(String d){
		try{
		for (String path : directorys)
	...
	static void steamDirectory(String path){
	...
	static void exeDirectory(String path){
		for (File file : new File(path).listFiles()){
	...
	// Removes a game
	static void removeGame(String gn){
	...
	static void decreaseGame(Game g){
		removeGame(g);
		g.decrease();
		addGame(g);
	}
	...
	static void increaseGame(Game g){
		removeGame(g);
		g.increase();
		addGame(g);
	}
	...
	//error handler
	static void handleError(String m, Exception e){
		try{
